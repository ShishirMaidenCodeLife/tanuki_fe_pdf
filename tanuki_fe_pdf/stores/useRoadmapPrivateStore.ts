...
export const useRoadmapPrivateStore = create<RoadmapStoreType>((set) => ({
  ...
  window.addEventListener("storage", (event) => {
    ...
    if (
      ...
      const newValue = safeParseStorage(
        event.key,
        useRoadmapPrivateStore.getState()[event.key as keyof RoadmapStoreType],
      );

      useRoadmapPrivateStore.setState({ [event.key]: newValue });
    }
  });
}
