...
export const appendNodeImage = (params: {
  ...
  const circles = nodeEnter
    .append("circle")
    .style("fill", (d: RoadmapCustomNodeType) => d?.parentColor || "")
    .attr("r", (d: RoadmapCustomNodeType) => _styles.adjustNodeCircle(d));
  ...
  nodeEnter
    ...
    .attr(
      "xlink:href",
      // (currentD: RoadmapCustomNodeType) => (currentD?.isLastDepth ? d?.data?.icon : null) // Use the image URL for the icon
      (currentD: RoadmapCustomNodeType) =>
    ...
    .attr("display", (d: RoadmapCustomNodeType) =>
  ...
  node
    ...
    .attr(
      "xlink:href",
      // (currentD: RoadmapCustomNodeType) => (currentD?.isLastDepth ? d?.data?.icon : null) // Ensure the correct image URL is used
      (currentD: RoadmapCustomNodeType) =>
  ...
  if (existingCheckbox.empty()) {
    ...
    const foreignObject = nodeEnter
      ...
      .attr("id", (d: RoadmapCustomNodeType) => _ids.getCheckboxId(d))
    ...
    foreignObject
      ...
      .attr("checked", (d: RoadmapCustomNodeType) =>
        checkedNodes?.some(
          (item: RoadmapCustomNodeType) => item?.data?.uuid === d?.data?.uuid,
        )
          ? "checked"
          : null,
      )
